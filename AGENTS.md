...
The `/history` page is a separate entry point (`history.html` + `history-page.js`) with no WebSocket/live-state dependency — it fetches `/api/history` once on load and renders a date-range banner, narrative callouts, hour/day-of-week activity charts, a day×hour heatmap, a top-attackers table, country/city breakdown, an Attack Techniques panel (MITRE ATT&CK tactic/technique leaderboard), protocol/ISP/duration charts, and a malware/threat-intel panel (per-service verdict counts, VirusTotal/URLhaus links, plus a list of IOCs observed in attacker command text). Clicking a country row, ISP row, or hour/day-of-week bar cross-filters the attackers table or highlights the heatmap client-side (no re-fetch — `lastData` is cached in the module). It does not touch `state.js`/`shared`/`ws.js`; the live dashboard (`index.html`) links to it instead of the old in-place "All-time" toggle (removed).
